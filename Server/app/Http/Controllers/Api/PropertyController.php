...
class PropertyController extends Controller
{
    ...
    private function applySortOrder(Builder $query, Request $request): Builder
    {
        ...
        if (in_array($sortOrder, ['asc', 'oldest', 'terlama'], true)) return $query->orderBy('created_at', 'asc');
        return $query->latest();
    }
    ...
    private function getRecommendationFacilityValue(Property $property): float
    {
        ...
        if (!$detail) return 0;
        return (float)($detail->bedrooms ?? 0) * 1.0 + (float)($detail->bathrooms ?? 0) * 0.85 + (float)($detail->living_rooms ?? 0) * 0.65 + (float)($detail->kitchens ?? 0) * 0.8 + (float)($detail->floors ?? 0) * 0.45 + ((bool) $detail->carport ? 1 : 0) * 0.7 + ((bool) $detail->garden ? 1 : 0) * 0.55 + ((bool) $detail->one_gate_system ? 1 : 0) * 0.75 + ((bool) $detail->security_24jam ? 1 : 0) * 0.8;
    }
    ...
    private function calculateRecommendationScore(Property $property, array $stats, array $weights): array
    {
        ...
        $score = $priceScore * ($weights['price'] / 100) + $locationScore * ($weights['location'] / 100) + $areaScore * ($weights['area'] / 100) + $facilityScore * ($weights['facilities'] / 100);
        return ['score' => $score, 'detail' => ['price_score' => $priceScore, 'location_score' => $locationScore, 'area_score' => $areaScore, 'facility_score' => $facilityScore]];
    }
    ...
    public function recommendations(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        if ($request->filled('search')) $query->where(function ($q) use ($request) {
            $q->where('title', 'like', "%{$request->search}%")->orWhere('description', 'like', "%{$request->search}%")->orWhere('kecamatan', 'like', "%{$request->search}%");
        });
        if ($request->filled('kecamatan')) $query->where('kecamatan', 'like', "%{$request->kecamatan}%");
        if ($request->filled('bedrooms')) $query->whereHas('detail', function ($q) use ($request) {
            $q->where('bedrooms', '>=', (int) $request->bedrooms);
        });
        if ($request->filled('bathrooms')) $query->whereHas('detail', function ($q) use ($request) {
            $q->where('bathrooms', '>=', (int) $request->bathrooms);
        });
        if ($request->filled('living_rooms')) $query->whereHas('detail', function ($q) use ($request) {
            $q->where('living_rooms', '>=', (int) $request->living_rooms);
        });
        if ($request->filled('kitchens')) $query->whereHas('detail', function ($q) use ($request) {
            $q->where('kitchens', '>=', (int) $request->kitchens);
        });
        if ($request->filled('floors')) $query->whereHas('detail', function ($q) use ($request) {
        ...
        if ($request->filled('water')) $query->whereHas('detail', function ($q) use ($request) {
            $q->where('water', $request->water);
        });
        if ($request->filled('listrik_type')) $query->whereHas('detail', function ($q) use ($request) {
        ...
        if ($request->filled('amenities')) {
            ...
            foreach ($amenities as $amenity) {
                ...
                if (!empty($amenity)) $query->whereHas('detail', function ($q) use ($amenity) {
        ...
        $weights = ['price' => max(0, (float) $request->input('price_weight', 35)), 'location' => max(0, (float) $request->input('location_weight', 30)), 'area' => max(0, (float) $request->input('area_weight', 20)), 'facilities' => max(0, (float) $request->input('facilities_weight', 15))];
        $totalWeight = array_sum($weights);
        if ($totalWeight <= 0) $weights = ['price' => 25, 'location' => 25, 'area' => 25, 'facilities' => 25];
        else {
        ...
        $stats = ['min_price' => (float) $prices->min(), 'max_price' => (float) $prices->max(), 'min_area' => (float) $areas->min(), 'max_area' => (float) $areas->max(), 'min_facility' => (float) $facilities->min(), 'max_facility' => (float) $facilities->max(), 'min_location_count' => (float) $locationCounts->min(), 'max_location_count' => (float) $locationCounts->max(), 'location_counts' => $locationCounts->all()];
    ...
    public function show(string $slug): \Illuminate\Http\JsonResponse
    {
        ...
        if (!request()->user()?->isAdmin()) {
            if ($property->status !== 'published' || !$property->is_verified) return response()->json(['message' => 'Not found'], 404);
            $this->stripPrivateFields($property);
        } else {
            $this->appendDocumentUrls($property);
        }
        if (!request()->user()?->isAdmin()) $property->increment('views');
    ...
    public function store(Request $request): \Illuminate\Http\JsonResponse
    {
        try {
            ...
            $validated = $request->validate([
                ...
                'building_type' => 'nullable|string|max:50', // ✅ UBAH KE STRING
            ...
            if ($uploadedCount === 0 && $request->hasFile('images')) Log::warning('⚠️ Images uploaded to R2 but NOT saved to database! Count: 0');
    ...
    {
        try {
            if (!$request->user()->isAdmin() && $property->user_id !== $request->user()->id) return response()->json(['message' => 'Unauthorized'], 403);
            ...
            $updatableFields = ['title', 'type', 'building_type', 'listing_type', 'kecamatan', 'city', 'price', 'certificate_type', 'certificate_status', 'status', 'description'];
            foreach ($updatableFields as $field) {
                if (isset($validated[$field])) $property->{$field} = $validated[$field];
            }
            if ($request->user()->isAdmin() && array_key_exists('is_verified', $validated)) $property->is_verified = (bool) $validated['is_verified'];
            ...
            if ($uploadedCount === 0 && $request->hasFile('images')) Log::warning('⚠️ Update: Images uploaded to R2 but NOT saved to database!');
            $property->load(['detail', 'images', 'user']);
            $this->appendImageUrls($property);
            $this->appendDocumentUrls($property);
    ...
    {
        if (!$request->user()?->isAdmin()) return response()->json(['message' => 'Unauthorized'], 403);
        $query = Property::with(['user', 'detail', 'images'])->where('is_verified', false)->orderByDesc('created_at');
    ...
    {
        try {
            if (!$request->user()->isAdmin() && $image->property->user_id !== $request->user()->id) return response()->json(['message' => 'Unauthorized'], 403);
            if (Storage::disk('s3')->exists($image->image_url)) Storage::disk('s3')->delete($image->image_url);
            $imageId = $image->id;
            $image->delete();
    ...
    private function handleDocumentUpload(Request $request, Property $property, string $field): void
    {
        ...
        if ($property->{$field}) {
            if (Storage::disk('s3')->exists($property->{$field})) Storage::disk('s3')->delete($property->{$field});
        }
    ...
    {
        $integerFields = ['luas_tanah', 'luas_bangunan', 'bedrooms', 'bathrooms', 'floors', 'kitchens', 'living_rooms', 'electricity_capacity'];
        foreach ($integerFields as $field) {
            if (array_key_exists($field, $detail)) {
                $value = $detail[$field];
                if ($value === 'null' || $value === '' || $value === null || (is_string($value) && trim($value) === '')) $detail[$field] = null;
            }
        }
        $booleanFields = ['carport', 'garden', 'one_gate_system', 'security_24jam'];
        foreach ($booleanFields as $field) {
            if (array_key_exists($field, $detail)) $detail[$field] = filter_var($detail[$field], FILTER_VALIDATE_BOOLEAN);
        }
        return $detail;
    }
    ...
    private function prepareDetailData(array $detail, int $propertyId): array
    {
        ...
        $integerFields = ['luas_tanah' => ['required' => true, 'default' => 0], 'luas_bangunan' => ['required' => false, 'default' => 0], 'bedrooms' => ['required' => false, 'default' => 0], 'bathrooms' => ['required' => false, 'default' => 0], 'floors' => ['required' => false, 'default' => 1], 'kitchens' => ['required' => false, 'default' => 0], 'living_rooms' => ['required' => false, 'default' => 0], 'electricity_capacity' => ['required' => false, 'default' => 0]];
        foreach ($integerFields as $field => $config) {
            $value = $detail[$field] ?? null;
            if ($value === null || $value === '' || $value === 'null') $data[$field] = $config['default'];
