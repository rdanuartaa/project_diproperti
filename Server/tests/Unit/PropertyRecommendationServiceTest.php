<?php

namespace Tests\Unit;

use App\Models\Property;
use App\Models\PropertyDetail;
use App\Services\Property\PropertyRecommendationService;
use PHPUnit\Framework\Attributes\Test;
use PHPUnit\Framework\TestCase;
use ReflectionClass;

class PropertyRecommendationServiceTest extends TestCase
{
    #[Test]
    public function it_converts_annual_rent_to_a_monthly_comparable_price(): void
    {
        $property = new Property([
            'listing_type' => 'sewa',
            'rent_period' => 'tahun',
            'price' => 12000000,
        ]);

        $this->assertSame(1000000.0, $this->invoke('getComparablePrice', $property));
    }

    #[Test]
    public function it_uses_fixed_reference_ranges_for_a_property_category(): void
    {
        $first = $this->invoke('buildStats', 'rumah', 'jual');
        $second = $this->invoke('buildStats', 'rumah', 'jual');

        $this->assertSame($first, $second);
        $this->assertSame(100000000, $first['min_price']);
        $this->assertSame(5000000000, $first['max_price']);
    }

    #[Test]
    public function it_penalizes_incomplete_data_without_excluding_the_property(): void
    {
        $property = new Property([
            'type' => 'kos',
            'listing_type' => 'sewa',
            'rent_period' => 'bulan',
            'price' => 1000000,
            'latitude' => -8.17211,
            'longitude' => 113.69953,
        ]);
        $property->setRelation('detail', new PropertyDetail([
            'total_rooms' => 10,
            'bathrooms' => 5,
        ]));

        $result = $this->invoke(
            'calculateScore',
            $property,
            $this->invoke('buildStats', 'kos', 'sewa'),
            ['price' => 25, 'location' => 25, 'area' => 25, 'facilities' => 25],
        );

        $this->assertContains('panjang_ruangan', $result['detail']['missing_fields']);
        $this->assertContains('lebar_ruangan', $result['detail']['missing_fields']);
        $this->assertSame(0.1, $result['detail']['completeness_penalty']);
        $this->assertGreaterThanOrEqual(0, $result['score']);
    }

    private function invoke(string $method, mixed ...$arguments): mixed
    {
        $reflection = new ReflectionClass(PropertyRecommendationService::class);
        $service = $reflection->newInstanceWithoutConstructor();
        $method = $reflection->getMethod($method);

        return $method->invoke($service, ...$arguments);
    }
}
