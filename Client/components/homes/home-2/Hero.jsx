  ...
  useEffect(() => {
    const fetchFiltersData = async () => {
      try {
        ...
        const uniqueTypes = [...new Set(items.map((p) => p.type).filter(Boolean))];
        setPropertyTypes(["Tipe Properti", ...uniqueTypes]);

        const uniqueCities = [...new Set(items.map((p) => p.city).filter(Boolean))];
    ...
    if (listingType) setActiveItem(listingType === "jual" ? "Dijual" : "Disewa");
            ...
            <div className="content-inner">
              <div className="heading-title">
                <h1 className="title">
                  ...
                  <span className="text-color-primary">Lebih Mudah</span> di DiProperti
                </h1>
                <p className="h6 fw-4">
